---
title: "Cache Invalidation Makes Your Code Fast: Building an SPSC Queue"
...
# Cache Invalidation Makes Your Code Fast: Building an SPSC queue.
...
If we make a 512-slot ring buffer where each slot is exactly 64 bytes (one cache line), the entire working set is 32 KB. That fits in L1. After warmup, the working set never leaves cache. The lines just bounce between cores, carried by the coherence protocol, never touching main memory.
