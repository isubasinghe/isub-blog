...
The performance of this queue is mostly due to aspects outside your direct control and that should make you feel uncomfortable as a programmer. But unfortunately, instead of us being given the capability to control the underlying hardware, we are instead given a leaky abstraction of said hardware.
...
Your CPU doesn't execute one instruction at a time. It has a pipeline — fetch, decode, execute, memory, writeback — and it overlaps them so multiple instructions are in flight at once. Think of it like a factory assembly line. While one instruction is being executed, the next one is being decoded, and the one after that is being fetched. When everything flows, you get close to one instruction retired per cycle.

A pipeline stall is when something blocks the line. The most common cause in our case: a load instruction needs data that isn't in L1 cache. The core issues the load, but the data has to come from somewhere else — another core's cache, L3, or worse, DRAM. Until that data arrives, anything that depends on it just sits there. The pipeline has a hole in it.

Out-of-order execution can hide some of this by doing unrelated work while waiting, but in a tight loop like our queue there's barely any independent work to schedule. The load feeds directly into a comparison that decides what happens next. So the core stalls.

The naive ring buffer has this problem. Every queue operation does 2 atomic loads and 1 atomic store. One of those loads is cross-core, it has to snoop a cache line from the other core's L1 over the interconnect. That's a stall. ~40-70 cycles of the core doing nothing, waiting for the coherence protocol to hand over the data.

The fix: cache the other side's counter locally. The writer keeps a `tail_cache`, just a plain integer, not atomic. The reader keeps a `head_cache`. You check the local copy first. If it says the queue isn't full (or empty), you skip the cross-core load entirely. You only pay for the snoop when the cached value is stale enough to give a wrong answer.

When does the cache give a wrong answer? Only when the queue is nearly full or nearly empty. With a 512-slot buffer that almost never happens in steady state. Cross-core loads drop from one per operation to roughly one per 512 operations.
